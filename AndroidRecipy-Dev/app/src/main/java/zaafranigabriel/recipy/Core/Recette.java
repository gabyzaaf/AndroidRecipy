...
public class Recette {
    ...
    public JSONArray getRecette(int fid){
        ...
        try {
            ...
            hashValue.put("url", "http://192.168.0.35/recipy/WSgetRecettes.php");
    ...
    public JSONArray getRecetteInfo(int fid, String titleRec){
        JSONArray json = null;
        try {
            String value;
            HashMap<String, String> hashValue = new HashMap<>();
            hashValue.put("url", "http://192.168.0.35/recipy/WSgetRecettesInfo.php");
            hashValue.put("fid", String.valueOf(fid));
            hashValue.put("title", titleRec);
            value = new WsGeneral().execute(hashValue).get();
            json = WsGeneral.getJsonArray(value);
        }catch(Exception e){
            // create log
    ...
    public Boolean deleteRecette(int fid,String title){
        try{
            String value;
            HashMap<String, String> hashValue = new HashMap<>();
            hashValue.put("url", "http://192.168.0.35/recipy/WSdeleteRecette.php");
            hashValue.put("fid", String.valueOf(fid));
